...
public class ChariotFly : MonoBehaviour
{
    ...
    void Update()
    {
        TankScript scr = King.GetComponent<TankScript>();
        if (Cooldown == false)
        { 
            if (Input.GetKeyDown(KeyCode.Space) && gameObject.GetComponent<TankScript>().isPlayer2Input == false)
            {  
                Rigidbody2D theChariot = Chariot.GetComponent<Rigidbody2D>();
                Cooldown = true;
                timeBtwShots = cd;
                GameObject shotChariot = Instantiate(Chariot, new Vector2(transform.position.x, transform.position.y), transform.rotation);
